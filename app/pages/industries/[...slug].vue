...
const { data: page } = await useAsyncData(route.path, () => {
  ...
  return queryCollection('industries').where('stem', '=', stem).first()
})

useSeoMeta({
  title: () => `${page.value?.title || 'Industry'} Solutions | Trevidia`,
...
<template>
  <div class="bg-gray-50 min-h-screen">
    <div v-if="page">
      <section class="bg-[#1a1a1a] text-white pt-24 pb-32 px-4 relative overflow-hidden">
        <div class="max-w-4xl mx-auto relative text-center">
          <NuxtLink to="/industries" class="inline-flex items-center gap-2 text-[#FFC162] hover:text-white transition-colors text-sm font-bold uppercase tracking-wider mb-8">
            &larr; All Industries
          </NuxtLink>
      ...
      <section class="max-w-5xl mx-auto px-4 -mt-16 relative pb-24">
        <div class="bg-white rounded-[2.5rem] p-8 md:p-16 shadow-xl border border-gray-100">
          <div class="grid md:grid-cols-2 gap-16 mb-16">
            <div>
              <h2 class="text-3xl font-bold text-[#1a1a1a] mb-6">Industry Advantages</h2>
              <ul class="space-y-4">
            ...
            <div class="bg-gray-50 p-8 rounded-3xl border border-gray-100">
              <h3 class="text-xl font-bold mb-4">Start your transformation</h3>
              <p class="text-gray-600 text-sm mb-6">Our experts in {{ page.title.toLowerCase() }} are ready to analyze your challenges and build a roadmap.</p>
          ...
          <div>
            <h2 class="text-3xl font-bold text-[#1a1a1a] mb-10 text-center">Vertical Capabilities</h2>
    ...
    <div v-else class="min-h-[60vh] flex flex-col items-center justify-center text-center px-4">
      <h2 class="text-4xl font-bold text-[#1a1a1a] mb-4">Focus Area Not Found</h2>
      <NuxtLink to="/industries" class="px-8 py-4 bg-[#FFC162] text-[#1a1a1a] font-bold rounded-xl shadow-lg mt-6">
        Back to Industries
      </NuxtLink>
    </div>
  </div>
